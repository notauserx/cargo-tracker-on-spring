package com.practicalddd.cargotracker.bookingservice.application.internal.outboundservices;

import com.practicalddd.cargotracker.bookingservice.infrastructure.brokers.rabbitmq.CargoEventSource;
...
public class CargoEventPublisherService {
    ...
    public void handleCargoBookEvent(CargoBookedEvent cargoBookedEvent) {
        try {
            ...
            cargoEventSource.cargoBooking().send(
                    MessageBuilder.withPayload(cargoBookedEvent).build()
            );
        } catch(Exception ex) {
            ex.printStackTrace();
        }
